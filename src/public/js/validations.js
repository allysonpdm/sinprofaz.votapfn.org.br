...
async function getSufragio(){
    let url = `/api/sufragios/${urlParams.get('votacao')}`;
    return fetch(`${url}`, {
        method: 'GET',
        headers: {
            'Accept': 'application/json',
...
let getAssociado = async() =>{
    ...
    data = {
        "sufragioId": urlParams.get('votacao'),
        "cpf": getCpf().value
    };
...
let setVotacao = async ()=>{
    ...
    if(response.status != 200){
        ...
        element = document.querySelector(`[data-erro="votacao"]`)
...
let setTitle = (title)=>{
    elements = document.querySelectorAll('[data-votacao]');
...
let setSubtitle = (subtitle)=>{
    ...
    elements = document.querySelectorAll('[data-subtitle]');
    elements.forEach((value, key)=>{
...
let setDescription = (description)=>{
    ...
    elements = document.querySelectorAll('[data-description]');
    elements.forEach((value, key)=>{
...
let setLinks = (arquivos)=>{
    ...
    elements = document.querySelectorAll('[data-links]');
    let p = document.createElement('p');
